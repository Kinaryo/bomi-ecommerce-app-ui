...
export default function OrderPage() {
  ...
  const tabRefs = useRef<Record<Status, HTMLButtonElement | null>>(
    {} as Record<Status, HTMLButtonElement | null>
  );
  const token =
    typeof window !== 'undefined' ? localStorage.getItem('token') : null;
    ...
    const fetchOrders = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/user/customer/order`,
          {
  ...
  const handlePayNow = (order: Order) => {
    ...
    (window.snap as any).pay(order.paymentToken, {
      ...
      onError: () =>
        showAlert('Error', 'Terjadi kesalahan saat pembayaran.', 'error'),
      onClose: () =>
        showAlert(
          'Dibatalkan',
  ...
  const handleBuyAgain = async (order: Order) => {
    try {
      ...
      const addCartPromises = order.items.map((item, idx) =>
        fetchWithAuth(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/user/customer/add-cart-item/${item.idProduct}`,
          {
            method: 'POST',
      ...
      const results: { status: string; data: { idCartItem: string } }[] =
      ...
      console.error(err);
      showAlert('Error', (err as Error).message || 'Terjadi kesalahan', 'error');
    }
  };
    ...
    <div className="p-6 md:p-10 mt-20 max-w-6xl mx-auto">
      ...
      {!loading &&
        filteredOrders.map((order) => (
          <div
            key={order.idOrder}
            className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition p-6 mb-6 space-y-4"
          >
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
              ...
              <span
                className={`px-3 py-1 rounded-md text-xs font-medium self-start md:self-auto ${
            ...
            <div className="bg-gray-50 p-4 rounded-xl">
              {order.shipping?.airWayBill && (
                <h4 className="font-semibold">
                  Resi: {order.shipping.airWayBill}
                </h4>
              )}
