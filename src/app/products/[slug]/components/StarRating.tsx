...
export default function StarRating({ rating }: { rating: number }) {
  // pastikan rating valid dan di antara 0-5
  const safeRating = Math.min(Math.max(0, Number(rating) || 0), 5);
  ...
  return (
    <div className="flex items-center text-yellow-400">
      ...
      <span className="ml-1 text-sm text-gray-600 font-semibold">{safeRating.toFixed(1)}</span>
    </div>
  );
}
