...
// tipe untuk hasil update profile API
interface UpdatedProfile {
  name: string;
  email: string;
  phone: string;
  [key: string]: unknown; // jaga-jaga bila API mengembalikan field lain
}
...
export default function EditProfileModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (errMsg) {
      Swal.fire("Validasi", errMsg, "warning");
      return;
    }
    ...
    try {
      ...
      const data: { data: UpdatedProfile; message?: string } = await res.json();
      if (!res.ok) {
      ...
      if (onSuccess) {
        onSuccess(data.data || { name, email, phone });
      }
      onClose();
    } catch (err) {
      const message =
      ...
      {isOpen && (
        ...
        >
          ...
          <motion.form
            ...
            className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[92%] max-w-xl bg-white rounded-2xl p-5 shadow-lg z-[10000]"
