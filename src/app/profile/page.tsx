...
import { Camera, Trash2, Loader2, LogOut, ChevronRight, User } from "lucide-react";
...
export default function ProfilePage() {
      ...
      if (data.status === "success") {
        ...
        if (addressData.status === "success" && addressData.data.length > 0) {
          const primaryAddress =
            addressData.data.find((addr: any) => addr.isPrimary) ||
            addressData.data[0];
  ...
  const handleDeleteProfileImage = async () => {
    ...
    try {
      ...
      if (data.status === "success") {
        setProfile((prev: any) => ({ ...prev, profileUrl: null }));
      } else {
  ...
  const handleSwitchToSeller = async () => {
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        // window.location.reload(); // reload otomatis setelah switch sukses/
      ...
      >
            ...
            {profile.profileUrl ? (
              ...
              <>
                ...
                <div
                  ...
                  onClick={handleUploadClick} // klik ikon langsung upload
                >
                  <User className="w-16 h-16 text-gray-300" />
                </div>

                {/* Tombol upload tetap ada di bawah ikon */}
        ...
        {profile.role === "customer" && (
          <div className="text-center">
            ...
            >
              {switching ? "Memproses..." : "Ingin Mulai Berjualan ? Klik Disini"}
            </button>
          </div>
        )}
