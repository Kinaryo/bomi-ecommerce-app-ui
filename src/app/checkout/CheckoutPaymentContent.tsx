...
import { MapPinHouse, Store, ChevronDown, ChevronUp, Truck, StickyNote } from 'lucide-react';
...
// ==================== START COMPONENT ====================
export default function CheckoutPaymentContent() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const itemsParam = searchParams.get('items') || searchParams.get('idCartItem');
  const cartItemIds = useMemo(() => itemsParam?.split(',').map(Number) || [], [itemsParam]);
  ...
  const fetchCheckout = useCallback(async () => {
    if (!token) {
      showAlert('Belum Login', 'Silakan login terlebih dahulu.', 'warning', () => router.push('/login'));
      return;
    }
    ...
    try {
      const payload = { cartItems: cartItemIds.map((id) => ({ idCartItem: id })) };
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/user/customer/checkout`, {
        method: 'POST',
        headers: {
      ...
      const hasShippingAddress = data?.shippingAddress?.addressCustomer?.trim().length > 0;
  ...
  useEffect(() => {
    ...
    else showAlert('Tidak ada produk', 'Tidak ada produk untuk checkout.', 'warning');
  }, [cartItemIds.length, fetchCheckout]);

  // =================== SHIPPING OPTIONS ===================
  const allShippingOptions: ShippingOption[] =
    checkoutData?.ongkir?.flatMap((o) => o?.data?.data.map((service) => ({ ...service, courier: o.courier })) || []) || [];

  const totalFinal = checkoutData ? checkoutData.payment.totalAllPriceFinal + (selectedShippingCost || 0) : 0;

  // =================== HANDLE PAYMENT ===================
  const handlePayment = async () => {
    if (!selectedCourier) {
      showAlert('Kurir Belum Dipilih', 'Silakan pilih pengiriman terlebih dahulu.', 'warning');
      return;
    }
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/user/customer/checkout/post`, {
        ...
        body: JSON.stringify({
          cartItems: checkoutData!.products.map((p) => ({ idCartItem: p.idCartItem, quantity: p.quantity })),
  ...
  const { products, payment, shippingAddress, storeName, shortAddressStore } = checkoutData;

  // =================== JSX ===================
  return (
    <div className="mx-auto p-8 mt-20 space-y-6 pb-28 border-gray-400">
      <h1 className="text-3xl font-extrabold text-center text-gray-800 mb-6">Checkout</h1>
      ...
      <div className="bg-white shadow-md rounded-2xl p-5 border-gray-200">
        ...
        <div className="px-8">
          <p className="text-gray-600 text-sm leading-snug">{shippingAddress?.addressCustomer}</p>
        </div>
      </div>
      ...
      <div className="bg-white shadow-md rounded-2xl p-5 border-gray-100">
        <div>
          ...
          <div className="px-8">
            <p className="text-gray-600 text-sm leading-snug">{shortAddressStore}</p>
          </div>
        </div>
        <div className="px-8 pt-2 space-y-2">
          {products.map((p) => (
            <div key={p.idProduct} className="flex items-center py-3 border-gray-400 shadow-md rounded-b-sm p-4">
              <Image src={p.imageUrl} alt={p.name} width={64} height={64} className="w-16 h-16 rounded-lg object-cover mr-4" />
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-800">{p.name}</p>
                <p className="text-xs text-gray-500">x{p.quantity}</p>
              </div>
              <p className="text-sm font-semibold text-black">Rp{p.totalPrice.toLocaleString('id-ID')}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white shadow-md rounded-2xl p-5 border-gray-200">
        ...
        >
          ...
          {showShippingOptions ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
        </button>

        {selectedCourier && (
          <div className="mt-3 p-3 bg-blue-50 rounded-xl border border-blue-200">
            <p className="text-sm font-medium text-gray-700">
              Kurir dipilih: <span className="font-semibold text-black">{selectedCourier.name}</span>
            </p>
            <p className="text-xs text-gray-500">Estimasi: {selectedCourier.etd}</p>
            <p className="text-sm font-semibold text-gray-800">Rp{selectedCourier.cost.toLocaleString('id-ID')}</p>
          </div>
        )}
        ...
        {showShippingOptions && (
          <div className="space-y-2 mt-3">
            {allShippingOptions.length === 0 ? (
              <p className="text-red-500 text-sm">Tidak ada opsi ongkir tersedia. Coba lagi nanti.</p>
            ) : (
              allShippingOptions.map((opt, idx) => (
                <label
                  key={idx}
                  className={`flex justify-between items-center py-2 px-3 rounded-xl border cursor-pointer ${
                    selectedCourier?.cost === opt.cost ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <p className="font-semibold text-sm text-gray-800">Rp{opt.cost.toLocaleString('id-ID')}</p>
                    <input
                      type="radio"
                      name="shipping"
                      value={opt.cost}
      ...
      <div className="bg-white shadow-md rounded-2xl p-5 border-gray-200 space-y-2">
        ...
        <div className="flex justify-between text-sm">
          <span>Diskon</span>
          <span className="text-green-600">- Rp{payment.totalDiscount.toLocaleString('id-ID')}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span>Pajak</span>
        ...
        <div className="flex justify-between text-sm">
          <span>Ongkir</span>
          <span>{selectedShippingCost > 0 ? `Rp${selectedShippingCost.toLocaleString('id-ID')}` : '-'}</span>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-xl font-bold text-blue-600">Rp{totalFinal.toLocaleString('id-ID')}</p>
        </div>
        <button
